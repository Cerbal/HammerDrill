package com.shumeau.hammerdrill.optimization;
...
public class LBFGS
{
	...
	{
		private static final long serialVersionUID = 8614378045279449659L;
		public int iflag;
...
//			ftol= 1.e-5;
		...
		while ( true )
		{
			if ( execute_entire_while_loop )
			{
				...
				{
					if ( iter > m ) bound = m;
					ys = ddot ( n , w , iypt + npt , 1 , w , ispt + npt , 1 );//å�?‘é‡�?yä¸Žå�?‘é‡�?sçš„ç‚¹ç§¯
					if ( ! diagco )
					{
						yy = ddot ( n , w , iypt + npt , 1 , w , iypt + npt , 1 );//å�?‘é‡�?yè‡ªå·±ä¸Žè‡ªå·±çš„ç‚¹ç§¯

						for ( i = 1 ; i <= n ; i += 1 )
						{
							diag [ i -1] = ys / yy;//è®¡ç®—ä¸‹ä¸€æ¬¡è¿­ä»£åˆ�?å§‹çš„H0
						}
					}
					else
					{
